/* ========================================================================== */
/*	    Exp22_1.c : 푸시푸시(PushPush) 게임 프로그램		      */
/* ========================================================================== */
...
#define Down		KEY3			// direction by KEY3
#define Up		KEY4			// direction by KEY4

#define KEY5		0x55			// previous stage icon of touch screen
#define	KEY6		0x66			// next stage icon of touch screen
#define	KEY7		0x77			// clear icon of touch screen
...
unsigned int  xo, yo;				// current x, y position

/* ---------------------------------------------------------------------------- */

prog_uchar board[Stage_max][12][14]={		/* define board of each stage */
	{ "              ",			// stage 1
	  "              ",
	  "   XXXXXXXX   ",
	  "   XX@ XXXX   ",
...
int main(void)					/* main routine of PushPush game */
{
  ...
  while(1)
	...
	for(x = 0; x < 14; x++) 
	  { buffer[y][x] = pgm_read_byte(&board[stage][y][x]);
	    if(buffer[y][x] == '@') 
	      { xo = x;
	        yo = y;
	        buffer[y][x] = ' ';
...
prog_uint16_t Symbol[6][16] = {					/* define board symbol */
...
prog_uint16_t Arrow[4][64] = {					/* define arrow of touch screen */
...
void Draw_arrow(U16 xPos,U16 yPos,U16 color,U08 code)	/* draw 32x32 arrow */
{
  ...
  for(y = 0; y < 64; y += 2)
    { TFT_GRAM_address(xPos,yPos+(y/2));
      font = ((U32)pgm_read_word(&Arrow[code][y]) << 16) | pgm_read_word(&Arrow[code][y+1]);
      for(x = 0; x < 32; x++)
...
void PushPush(U16 direction)			/* push box by direction arrow */
{
  ...
  if(buffer[yo+my][xo+mx] != 'X')		// 이동할 곳이 벽이 아닌 경우에만
	...
	{ if((buffer[yo+my*2][xo+mx*2] == ' ') || (buffer[yo+my*2][xo+mx*2] == '0')) // 박스가 갈 곳
	    { if(pgm_read_byte(&board[stage][yo+my][xo+mx]) == '0')
...
unsigned char Check_OK(void)			/* check game stage OK */
{
  ...
  for(y = 0; y < 12; y++)
    for(x = 0; x < 14; x++)
      if((pgm_read_byte(&board[stage][y][x]) == '0') && (buffer[y][x] != '*'))
	return 0;
